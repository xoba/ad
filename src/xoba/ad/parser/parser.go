...
func Run(args []string) {
	...
	code, err := Parse(main, pkg, private, templates, formula, 0.00001)
	if err != nil {
		log.Fatal(err)
	}
...
func Parse(main bool, pkg, private, templates, formula string, dx float64) ([]byte, error) {
	if len(private) == 0 {
...
`, private, formula, lex.lhs.S)
	...
	t := template.Must(template.New("output.go").Parse(`// autogenerated {{.time}}
