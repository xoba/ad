...
`, private, formula, lex.lhs.S)
	...
	fmt.Fprintf(pgm, "if !computeGrad {\n")
	{
		...
		fmt.Fprint(pgm, strings.Join(list, ","))
		fmt.Fprintln(pgm)
	}
	fmt.Fprintln(pgm, "}")

	indexedArgMap := make(map[string]int)
...
{{.decls}} 
	...
	c1, grad1,{{.ignoredReturnSig}} := {{.funcName}}(true,{{.vars}})
...
`))
	...
	t.Execute(f, map[string]interface{}{
		...
		"funcName":         name,
		"funcs":            code,
		"ignoredReturnSig": ignoredReturnSig,
	...
	case identifierNT, indexedIdentifierNT:
		if _, ok := v.vars[root.S]; !ok {
			...
			if root.Type == indexedIdentifierNT {
				...
				if root.I > v.maxIndicies[root.S] {
					v.maxIndicies[root.S] = root.I
				}
			}
			out = append(out, step)
		} else {
...
func sort2(m map[string]int) (out []string) {
	for k := range m {
		out = append(out, k)
	}
	sort.Strings(out)
